...
(() => {
  ...
  function tower(x, z, h = 5, color = 0x59636b) {
    // Tall sniper outpost with a platform and four rail posts.
  ...
  function building(x, z, w, d, h, color = 0x59636b, doorAxis = 'x') {
    // Four walls with a doorway gap in the middle of the front wall.
  ...
  function maze() {
    ...
    // Long alternating corridors with real turns and dead ends.
    const rows = [
  ...
  function sniperOutposts() {
    // Four elevated corners plus low cover between them.
  ...
  function militaryBase() {
    ...
    // Watchtowers and sandbag-style cover.
  ...
  function ruins() {
    // Broken stone streets and partial buildings.
  ...
  function harbor() {
    // Docks / shipping lanes represented by long platforms and containers.
  ...
  function mine() {
    // Deep-looking mine tunnels made from repeated support frames.
  ...
  function swamp() {
    // Boardwalks, stilt huts and broken bridges.
  ...
  function build(id) {
    ...
    else maze(); // Delta is a maze/urban combat layout by default.
    builtFor = map;
  }
  ...
  // game.js exposes its lexical socket/me/scene to later classic scripts.
  // Build after init so terrain exists before structures are placed.
  socket.on('init', start);
  setTimeout(start, 900);
  setInterval(()=>{ if(running && me && me.alive) pushOut(me); }, 30);

  // Keep authoritative move packets outside map walls too.
