...
// STATE
// ---------------------------------------------------------------------
const players = {};   // socketId -> {x,y,z,yaw,hp,score,name,color,alive,permaDead}
...
const reviveCards = []; // multiplayer only: {id,forId,forName,x,y,z,carriedBy,channelProgress,ready}
const REVIVE_STATIONS = [{ x: 8, z: 8 }, { x: WORLD - 8, z: WORLD - 8 }];   // opposite corners
...
// Revive a player back into the fight (used by solo self-revive, wave-end squad
// revive, and the full-wipe safety net). Keeps existing gear; resets hp/shield.
...
function hurtPlayer(pid, p, amount, by) {
  ...
  if (p.hp <= 0) {
    ...
    if (solo) {
      // Singleplayer: no revives at all, except a held self-revive device.
      if (p.items && p.items.selfRevive > 0) {
        p.items.selfRevive--;
        revivePlayer(pid, Math.round(PLAYER_MAX_HP * 0.5));
        io.emit("selfRevived", { id: pid });
      } else {
        p.permaDead = true;   // true death: no more chances this run
        io.emit("gameOver", { id: pid });
      }
    } else {
      // Multiplayer: drops a revive card a teammate must carry to a reviver station.
      reviveCards.push({
...
function killEnemy(e, by) {
  ...
  } else if (Object.keys(players).length === 1 && Math.random() < 0.02) {
    // Solo only, and rare: the one way to cheat death without a teammate around
    pickups.push({
      id: Math.random().toString(36).slice(2),
      x: e.x, y: e.y, z: e.z,
      kind: "revive",
...
io.on("connection", (socket) => {
  ...
  socket.on("disconnect", () => {
    ...
    for (let i = reviveCards.length - 1; i >= 0; i--) {
      if (reviveCards[i].forId === socket.id) reviveCards.splice(i, 1);
      else if (reviveCards[i].carriedBy === socket.id) reviveCards[i].carriedBy = null;
    }
...
setInterval(() => {
  // --- Revive cards (multiplayer): pick up, carry to a station, channel 5s ---
  const now = Date.now();
  for (const card of reviveCards) {
    if (card.ready) continue;
    if (card.carriedBy == null) {
      // Lying on the ground: any living player who walks over it picks it up
      for (const id in players) {
        const p = players[id];
      ...
      if (!carrier || !carrier.alive) {
        // Carrier died or disconnected mid-carry: card drops where they were, progress kept
        card.carriedBy = null;
  ...
  if (alivePlayers === 0 && Object.keys(players).length > 0) {
    // Full wipe: nobody left to carry a card or use a device — reset clean so the
    // run continues rather than soft-locking on an un-revivable team.
    ...
    if (budget === 0 && enemies.length === 0) {
      ...
      // Resolve this wave's revive cards: ready -> back in the fight; still
      // pending -> that teammate didn't make it in time and is out for good.
      for (let i = reviveCards.length - 1; i >= 0; i--) {
  ...
  io.emit("sync", {
    players, enemies, wave, pickups, blobs, grenades, reviveCards,
...
server.listen(PORT, () => console.log(`Server running on port ${PORT} (seed ${SEED})`)); (movement + shots) and render the synced state.
   ===================================================================== */
