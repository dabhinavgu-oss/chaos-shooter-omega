/* CHAOS SHOOTER OMEGA — REAL MAP GEOMETRY + STABLE RAMPS + DOORS */
(() => {
  ...
  function box(x,y,z,sx,sy,sz,color,solid=true){const m=new THREE.Mesh(new THREE.BoxGeometry(sx,sy,sz),mat(color,color===0x263238||color===0x37474f));m.position.set(x,y,z);group.add(m);if(solid)collision.push({type:'box',x:x-sx/2,z:z-sz/2,w:sx,d:sz,h:sy,y:y});return m;}
  ...
  // A mathematical ramp is used for tower stairs. It has NO overlapping step collisions,
  // so entering the stairs cannot cause the old vibration/jitter problem.
  function ramp(x,z,w,run,h,color=0x6d7478){
    const base=groundHeightAt(x,z);
    const mesh=new THREE.Mesh(new THREE.BoxGeometry(w,0.18,run),mat(color));
    mesh.position.set(x,base+h/2,z);
    mesh.rotation.x=Math.atan2(h,run);
    group.add(mesh);
    collision.push({type:'ramp',x:x-w/2,z:z-run/2,w,d:run,h,base,dir:-1});
  }
  ...
  function tower(x,z,h=6,color=0x59636b){
    ...
    // Wide stable ramp instead of stacked colliders.
    ramp(x,z+0.05,2.2,5.8,h,0x6d7478);
    // Rail posts are decorative/non-solid so they cannot trap the player.
    for(const [dx,dz] of [[-1.9,-1.9],[1.9,-1.9],[-1.9,1.9],[1.9,1.9]]) box(x+dx,baseY+h+.9,z+dz,.25,1.3,.25,0x263238,false);
    box(x,baseY+h+.85,z-2.45,2.5,.18,.18,0x263238,false);
    box(x,baseY+h+.85,z+2.45,2.5,.18,.18,0x263238,false);
  }

  function door(x,z,axis='x',open=true){
    const y=groundHeightAt(x,z), metal=0x263238, panel=0x8a9298;
    if(axis==='x'){
      box(x-1.25,y+1.35,z,.22,2.7,.22,metal,false);box(x+1.25,y+1.35,z,.22,2.7,.22,metal,false);box(x,y+2.62,z,2.72,.22,.22,metal,false);
      // Door panels sit to the sides, leaving a clear walk-through opening.
      if(open){box(x-1.55,y+1.25,z,.5,2.5,.12,panel,false);box(x+1.55,y+1.25,z,.5,2.5,.12,panel,false);}
      else box(x,y+1.25,z,.12,2.5,2.2,panel,true);
    }else{
      box(x,y+1.35,z-1.25,.22,2.7,.22,metal,false);box(x,y+1.35,z+1.25,.22,2.7,.22,metal,false);box(x,y+2.62,z, .22,.22,2.72,metal,false);
      if(open){box(x,y+1.25,z-1.55,.12,2.5,.5,panel,false);box(x,y+1.25,z+1.55,.12,2.5,.5,panel,false);}
      else box(x,y+1.25,z,.12,2.5,2.2,panel,true);
    }
  }

  function building(x,z,w,d,h,color=0x59636b,doorAxis='x'){
    wall(x-w/2,z,.5,d,h,color);wall(x+w/2,z,.5,d,h,color);
    if(doorAxis==='x'){
      // Back wall; front wall is split around a clearly visible open doorway.
      wall(x,z-d/2,w,.5,h,color);
      wall(x-w/2+1,z+d/2,Math.max(.5,w/2-1),.5,h,color);
      wall(x+w/2-1,z+d/2,Math.max(.5,w/2-1),.5,h,color);
      door(x,z+d/2,'x',true);
    }else{
      wall(x,z-d/2,w,.5,h,color);wall(x+w/2,z+d/2-1,.5,Math.max(.5,d/2-1),h,color);wall(x+w/2,z-d/2+1,.5,Math.max(.5,d/2-1),h,color);
      door(x+w/2,z,'z',true);
    }
  ...
  function mainGate(){
    const z=4, c=0x56616a;
    wall(24,z,14,2,3,c);wall(56,z,14,2,3,c);
    // Obvious military entrance: open center with gate pillars and overhead beam.
    wall(31,z,1.4,2,3,0x263238);wall(49,z,1.4,2,3,0x263238);
    box(40,groundHeightAt(40,z)+3.2,z,20,.5,2,0x263238,false);
    box(40,groundHeightAt(40,z)+2.2,z,10,.12,.12,0xb0b7bc,false);
  }
  ...
  function militaryBase(){
    const wc=0x56616a,b=0x3f4a52;
    // Perimeter with a real front gate opening instead of a solid wall.
    mainGate();wall(40,76,34,2,3,wc);wall(4,40,2,34,3,wc);wall(76,40,2,34,3,wc);
    building(40,22,18,12,5,b,'x');building(19,40,12,16,4,wc,'z');building(61,40,12,16,4,wc,'z');building(40,58,24,10,5,0x45515a,'x');
    // Guard posts, sniper towers, barriers and a clearly readable gatehouse layout.
    tower(10,10,5);tower(70,10,5);tower(10,70,5);tower(70,70,5);
    for(let x=30;x<=50;x+=5){crate(x,40,0x6d5838);crate(x,47,0x6d5838);}
    for(const q of [[12,32],[12,48],[68,32],[68,48]])wall(q[0],q[1],5,1.5,1,0x8a7958);
  }
  ...
  function pushOut(p){if(!p||!collision.length)return;for(let pass=0;pass<2;pass++)for(const c of collision){
    const r=.42;
    if(c.type==='ramp'){
      const minX=c.x-r,maxX=c.x+c.w+r,minZ=c.z-r,maxZ=c.z+c.d+r;
      if(p.x>minX&&p.x<maxX&&p.z>minZ&&p.z<maxZ){
        const t=Math.max(0,Math.min(1,(c.z+c.d-p.z)/c.d));
        const target=c.base+c.h*t;
        if(p.y < target+1.05) p.y=target+0.12;
      }
      continue;
    }
    const minX=c.x-r,maxX=c.x+c.w+r,minZ=c.z-r,maxZ=c.z+c.d+r;
    if(p.x>minX&&p.x<maxX&&p.z>minZ&&p.z<maxZ){
      const base=groundHeightAt(p.x,p.z),top=c.y+c.h/2;
